...
const CryptoCurrency = ({ props }) => {
    return (
        <div className={styles.currency}>
            ...
            <h2 className={styles.price}>{`$${props?.current_price}` || 'NaN'}</h2>
            <span
                className={classNames(
                    styles.percentage,
                    props?.price_change_percentage_24h?.toString().includes('-')
                        ? styles.percentage__negative
                        : styles.percentage__positive,
                )}>
                {props?.price_change_percentage_24h?.toString().includes('-')
                    ? props?.price_change_percentage_24h.toFixed(2)
                    : `+${props?.price_change_percentage_24h.toFixed(2)}` || 'NaN'}
